...
def main() -> None:
    ...
    include_derived = st.sidebar.checkbox("파생 특성(4개)을 모델 입력에 포함", value=False)
    ...
    if st.sidebar.button("예측 실행"):
        ...
        display_df["혈액상태"] = blood_health
        display_df["췌장상태"] = pancreas_health
        display_df["심혈관위험지수"] = cardiovascular_risk
        display_df["고령"] = elderly
        ...
        try:
            ...
            st.write("참고: 모델이 기대하는 특성 수(예: 8 또는 12)가 로드된 스케일러/모델과 일치하는지 확인하세요.")
            return
